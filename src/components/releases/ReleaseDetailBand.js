  ...
  return (
    <>
      <Link to={`release/${release.id}`}>
        <div className="pop-out inline-block m-2 border-2 border-none rounded-lg shadow-md p-4 backdrop-blur-md" key={`release--${release.id}`}>
          <AdvancedImage cldImg={releaseImage} />
          <div className="mt-2 text-center">
            <div>{release.title}</div>
            <div className="italic">{release.releaseType.type} - {new Date(release.releaseDate).toLocaleDateString("en-US", {year: "numeric", month: "long", day: "numeric"})}</div>
